...
History: 19JAN.00; Henk B&B; Definition.
	 ...
	 06OCT.03; Henk B&B; Added can_recv_descriptor_refresh();
	                     call it in can_read() as well as can_write(),
			     to refresh descriptors of all receiving buffers.
	 04MAR.04; Henk B&B; Enable the 81c91's Transmit Check feature.
	 04MAR.04; Henk B&B; Added toggle bit to Emergency message.
	 19MAY.04; Henk B&B; Check the integrity of one important register
	                     with comm settings: reinitialize CAN-controller
			     if not as expected;
			     initialize Emergency toggle bit globally, once,
			     not in can_init(), to prevent bit not toggling;
			     init error counter only when all is initialized.
--------------------------------------------------------------------------- */
...
const BYTE CAN_DESCRIPTOR[C91_MSG_BUFFERS][2] =
{
  ...
  /* Receive-PDO3 */
  { RPDO3_OBJ,		C91_RPDO3_LEN },

  /* Receive-PDO4 */
  { RPDO4_OBJ,		C91_RPDO4_LEN },
...
void can_write( BYTE object_no, BYTE len, BYTE *msg_data )
{
  ...
  addr += (len-1);
  for( byt=len-1; byt>=0; --byt, --addr ) can_write_reg( addr, msg_data[byt] );
...
void can_check_for_errors( void )
{
  ...
  /* Check the integrity of an important register with comm settings */
  {
    BOOL rst = FALSE;
    CAN_INT_DISABLE();
    if( can_read_reg( C91_OUTPUTCONTROL_I ) != 0x18 )
      if( can_read_reg( C91_OUTPUTCONTROL_I ) != 0x18 )
	rst = TRUE;
    CAN_INT_ENABLE();
	...
	++CanErrorCntr;
	can_init( FALSE );
	can_write_emergency( 0x00, 0x81, 0x00, 0x00,
...
static void can_load_config( void )
{
  ...
  if( storage_read_block( STORE_CAN, CAN_STORE_SIZE, block ) )
    {
      RtrDisabled        = block[0];
...
void canint_handler( void )
{
  ...
  if( object_no != NO_OBJECT )
      ...
      else
	{
	  ...
	  /* Copy data bytes */
	  addr += (dlc-1);
	  for( byt=dlc-1; byt>=0; --byt,--addr ) msg[byt] = can_read_reg(addr);
	}
