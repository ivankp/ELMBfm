...
static BYTE LifeTimeFactor; /* (copy in EEPROM) */
...
static BYTE HeartBeatTime;  /* (copy in EEPROM) */
...
BOOL guarding_store_config( void )
{
  ...
  return( storage_write_block( STORE_GUARDING, GUARDING_STORE_SIZE, block ) );
}
...
static void guarding_load_config( void )
{
  ...
  if( storage_read_block( STORE_GUARDING, GUARDING_STORE_SIZE, block ) )
    {
